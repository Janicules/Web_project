...
if(isset($_SESSION['account'])){
    ...
    $r = $cesi->fetch();
    if($r['Status'] == 1){
		$get_hoodies=$conn->prepare("SELECT * FROM `product` WHERE `product`.`Category`= 'Hoodie' ");
		$get_hoodies->execute();
	...
	echo'
		...
		<nav>
			...
			<a class="tab" style="color:#fff">Hoodies</a>
		...
		<script type="text/javascript">
			...
			function add(name, price){
				...
				location.href = \'addhoodies.php?nameprod=\'+name+\'&price=\'+price;
			}

		</script>
		';
			while ($row=$get_hoodies->fetch()){
